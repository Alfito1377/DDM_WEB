                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        <div class="flex items-center justify-center gap-2">
                            <button onclick="showQR({storeName: '{{ $toko->store_name }}', qrLoginUrl: '{{ $qrImageLogin }}', qrCheckpointUrl: '{{ $qrImageCheckpoint }}'})" class="px-3 py-1.5 border border-green-200 bg-green-50 text-green-700 rounded-lg text-xs font-bold hover:bg-green-100 transition">
                                QR Code
                            </button>
                            <button onclick="openEditModal({{ $toko->id }})" class="px-3 py-1.5 border border-blue-200 bg-blue-50 text-blue-700 rounded-lg text-xs font-bold hover:bg-blue-100 transition">
                                Edit
                            </button>
                            <button onclick="deleteStore({{ $toko->id }}, '{{ $toko->store_name }}')" class="px-3 py-1.5 border border-red-200 bg-red-50 text-red-700 rounded-lg text-xs font-bold hover:bg-red-100 transition">
                                Hapus
                            </button>
                        </div>
                    </td>
        ...
        <div class="p-6 overflow-y-auto">
            <form id="formRegisterCustomer" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="jenis_mitra_id" required class="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition bg-white">
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg transform transition-all overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto">
            <form id="formEditCustomer" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">Nama Toko</label>
                    <input type="text" id="edit_store_name" name="store_name" required class="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">Nama Pemilik Toko</label>
                    <input type="text" id="edit_owner_name" name="owner_name" required class="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">No. WhatsApp / Telepon</label>
                    <input type="number" id="edit_phone_number" name="phone_number" required class="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">Alamat Lengkap</label>
                    <textarea id="edit_address" name="address" rows="3" required class="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition resize-none"></textarea>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">Jenis Mitra</label>
                    <select id="edit_jenis_mitra_id" name="jenis_mitra_id" required class="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition bg-white">
...
<div id="qrModal" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white p-6 rounded-2xl shadow-xl max-w-xl w-full mx-4 transform transition-all">
        ...
        <div class="flex flex-col items-center gap-6 mb-6">
            ...
            <div class="flex flex-col items-center mb-4">
                <h1 class="font-bold text-lg text-gray-800">QR Code Checkpoint</h1>
                <img id="modalQrImageCheckpoint" src="" alt="QR Code Checkpoint" class="w-48 h-48 object-contain">
            </div>
        </div>
        <p class="text-xs text-center text-gray-500 mb-6">Scan QR ini menggunakan kamera perangkat toko untuk masuk otomatis dan checkpoint sopir untuk konfirmasi.</p>
        <a id="modalDownloadLoginBtn" href="#" target="_blank" class="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold my-2 py-2.5 rounded-xl transition">
            Buka Gambar QR Login di Tab Baru
        </a>
        <a id="modalDownloadCheckpointBtn" href="#" target="_blank" class="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold my-2 py-2.5 rounded-xl transition">
...
<script>
    ...
    async function deleteStore(id, storeName) {
        if (confirm(`Apakah Anda yakin ingin menghapus mitra "${storeName}"? Tindakan ini tidak dapat dibatalkan.`)) {
            try {
    ...
    function showQR({storeName, qrLoginUrl, qrCheckpointUrl}) {
        ...
        document.getElementById('modalDownloadLoginBtn').href = qrLoginUrl;
        document.getElementById('modalDownloadCheckpointBtn').href = qrCheckpointUrl;
