...
<div class="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-200/60 relative">
   ...
   <!-- ================= HEADER ================= -->
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-6 gap-4">
        ...
        <div class="w-full lg:w-auto">
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-1 leading-relaxed">
                Tinjau foto bukti dan ambil keputusan untuk pengajuan pengembalian produk.
            </p>
        </div>

        <!-- Counter Pending -->
        <div class="bg-amber-50 border border-amber-200/60 flex items-center gap-2.5 px-3 sm:px-4 py-2.5 rounded-xl shadow-sm w-full lg:w-auto">
            <span class="relative flex h-3 w-3 shrink-0">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                <span class="relative inline-flex rounded-full h-3 w-3 bg-amber-500"></span>
            </span>
            <span class="text-amber-700 text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                Menunggu Proses:
                <span class="text-amber-800 bg-amber-200/50 px-1.5 py-0.5 rounded ml-1">
                    {{ $pendingCount ?? 0 }}
                </span>
            </span>
        </div>
    ...
    <!-- FILTER & PENCARIAN -->
    <!-- ===================================================== -->
    <div class="mb-6 bg-slate-50/50 p-3 sm:p-4 rounded-xl border border-slate-200/60 shadow-sm">
        <form method="GET" action="{{ url()->current() }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div class="relative flex-1 w-full sm:max-w-md">
                ...
                <input type="text" name="search" value="{{ request('search') }}" 
                    ...
                    class="w-full bg-white border border-slate-300 text-slate-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 p-2.5 outline-none transition shadow-sm">
                
                <button type="submit" class="hidden"></button>
                
                <!-- Tombol Reset Search -->
                @if (request('search'))
                    <a href="{{ request()->fullUrlWithQuery(['search' => null]) }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 transition" title="Hapus pencarian">
            ...
            <select name="status" onchange="this.form.submit()" 
                class="w-full sm:w-auto bg-white border border-slate-300 text-slate-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 outline-none font-semibold cursor-pointer shadow-sm">
                    ...
                    <tr class="hover:bg-blue-50/20 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button
                                    onclick='openDetailModal(@json($retur))'
                                ...
                                @if($retur->status === 'Pending')
                                    ...
                                    <button
                                        onclick="processReturn({{ $retur->id }}, 'Approved')"
                                        title="Setujui Retur"
                                    ...
                                    <button
                                        onclick="processReturn({{ $retur->id }}, 'Rejected')"
                                        title="Tolak Retur"
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="p-4 bg-slate-50/70 border-b border-slate-100">
                ...
                <div class="flex items-start justify-between gap-3">
                    ...
                    @if($retur->status === 'Pending')
                        ...
                        <span class="shrink-0 inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 border border-amber-200 text-[9px] font-bold px-2 py-1 rounded-lg uppercase">
                    ...
                    @elseif($retur->status === 'Approved')
                        ...
                        <span class="shrink-0 inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[9px] font-bold px-2 py-1 rounded-lg uppercase">
                    ...
                    @else
                        ...
                        <span class="shrink-0 inline-flex items-center gap-1.5 bg-rose-50 text-rose-700 border border-rose-200 text-[9px] font-bold px-2 py-1 rounded-lg uppercase">
            ...
            <div class="p-4 space-y-4">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <span class="text-[11px] text-slate-600 font-medium bg-slate-100 border border-slate-200 px-2 py-1 rounded-md">
                    ...
                    <span class="text-[11px] text-slate-700 font-bold bg-blue-50 border border-blue-100 px-2 py-1 rounded-md">
                ...
                <div>
                    ...
                    @if($retur->notes)
                        ...
                        <div class="bg-slate-50 text-slate-600 px-3 py-2 rounded-lg border border-slate-100 text-xs leading-relaxed">
...
<div id="detailModal"
    ...
    <div id="modalContent"
        ...
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/80 backdrop-blur shrink-0">
        ...
        <div class="p-4 sm:p-6 overflow-y-auto custom-scrollbar">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-6">
                ...
                <div class="bg-slate-50/50 p-3 sm:p-4 rounded-xl border border-slate-100">
                    ...
                    <p id="modalStoreName"
                       class="text-sm font-bold text-slate-800 break-words">
