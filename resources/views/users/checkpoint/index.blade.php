            ...
            <div class="p-6">
                @if (isset($store) && $store)
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 text-sm mt-1 flex items-center gap-1">
                                    ...
                                    <span
                                        class="line-clamp-2 leading-tight">{{ $store->address ?? 'Alamat tidak tersedia' }}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="bg-amber-50 rounded-2xl p-4 mb-6 border border-amber-100">
                            ...
                            <div class="space-y-2">
                                @foreach ($shipments as $shipment)
                                    <label
                                        class="bg-white rounded-xl p-3 shadow-sm border border-amber-100 flex items-center gap-3 cursor-pointer hover:bg-amber-50/50 transition-colors">
                                        <input type="checkbox" name="selected_shipments[]"
                                            value="{{ $shipment->id_logistic }}"
                                            class="w-5 h-5 text-brand-600 rounded focus:ring-brand-500 border-slate-300 shipment-checkbox">
                                        <div class="flex-1">
                                            <p class="font-bold text-slate-800 text-xs">Resi:
                                                {{ substr($shipment->shipmentId, 0, 8) }}...</p>
                                            <p class="text-[10px] text-slate-500 mt-0.5">
                                                Kurir: <strong>{{ $shipment->driverName ?? 'Tidak diketahui' }}</strong>
                                                &bull;
                                                {{ $shipment->plateNo }} ({{ $shipment->vehicleType }})
                                            </p>
                                        </div>
                                        <span
                                            class="bg-amber-100 text-amber-700 py-1 px-2 rounded-lg text-[10px] font-bold">In
                                            Transit</span>
                                    </label>
                                @endforeach
                            </div>
                    ...
                    @else
                        <!-- No Active Shipments -->
                        <div
                            class="bg-slate-50 text-slate-500 p-4 rounded-2xl mb-6 border border-slate-200 text-center text-sm">
                ...
                <form id="checkpoint-form" class="space-y-5">
                    ...
                    <div class="flex items-center justify-between px-1 mb-2">
                        ...
                        <span id="live-clock"
                            class="text-sm font-bold text-brand-600 bg-brand-50 px-3 py-1.5 rounded-lg border border-brand-100">--:--:--</span>
                    </div>

                    <div>
                        <label for="nama-pengirim" class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Nama Kurir
                            / Pengirim <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <div
                            ...
                            <input type="text" id="nama-pengirim" name="nama_pengirim" required
                    ...
                    <div>
                        <label for="nomor-handphone" class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Nomor
                            Handphone / WA <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <div
                                class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <input type="tel" id="nomor-handphone" name="nomor_handphone" required
                    ...
                    <div>
                        <label for="catatan" class="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Nomor Plat
                            Kendaraan (Opsional)</label>
                        <div class="relative">
                    ...
                    <div class="pt-2">
                        <button type="submit" id="submit-button" @if (!isset($store) || !$store) disabled @endif
                            ...
                            <svg id="btn-icon" class="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M14 5l7 7m0 0l-7 7m7-7H3" />
                            </svg>
                            <svg id="btn-spinner" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white hidden"
