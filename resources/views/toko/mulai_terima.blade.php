...
<div class="w-full space-y-4 sm:space-y-6">
    ...
    <!-- Tombol Kembali -->
    <a href="{{ url('/toko/penerimaan') }}"
        class="inline-flex items-center text-xs sm:text-sm font-medium text-gray-500 hover:text-gray-800 transition-colors py-1">
    ...
    <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-5 mb-5 sm:mb-6">
            ...
            <div class="w-full lg:w-auto bg-gray-50 px-4 py-3 rounded-xl border border-gray-200">
                ...
                <div class="font-bold text-gray-800 text-base sm:text-lg flex items-center gap-1.5 flex-wrap">
                    ...
                    <span class="text-gray-400 font-normal text-xs sm:text-sm">
                        dari
                    </span>
                    ...
                    @if($isCompleted)
                        <svg class="w-5 h-5 text-green-500 ml-1 flex-shrink-0"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24">
        ...
        @if($isCompleted)
            ...
            <div class="bg-green-50 text-green-700 p-5 sm:p-6 rounded-2xl flex flex-col items-center justify-center text-center border border-green-200">
                ...
                <svg class="w-12 h-12 sm:w-16 sm:h-16 text-green-500 mb-3 sm:mb-4"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24">
                ...
                <p class="text-xs sm:text-sm opacity-90 max-w-md leading-relaxed">
                    Pengiriman ini sudah dibongkar sepenuhnya.
                    Status logistik telah otomatis diperbarui menjadi selesai.
                </p>

                <a href="{{ url('/toko/penerimaan') }}"
                    class="mt-5 sm:mt-6 bg-green-600 text-white px-5 sm:px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold hover:bg-green-700 transition shadow-sm">
                    Kembali ke Daftar
                </a>
                    ...
                    <div class="w-full max-w-md">
                        ...
                        <div class="relative w-full aspect-square sm:aspect-[4/3] lg:aspect-square rounded-2xl overflow-hidden bg-gray-100 border-4 border-gray-100 shadow-inner">
                            ...
                            <div id="scanner-placeholder"
                                class="absolute inset-0 flex flex-col items-center justify-center text-gray-400 pointer-events-none bg-white">

                                <svg class="w-12 h-12 sm:w-16 sm:h-16 mb-2 opacity-50"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center my-4 sm:my-5">
                            ...
                            <span class="px-3 text-[10px] sm:text-xs font-semibold text-gray-400 uppercase">
                                ATAU
                            </span>
                        ...
                        <form id="manual-scan-form"
                            ...
                            <input
                                ...
                                autocomplete="off"
                                class="w-full min-w-0 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all font-medium placeholder-gray-400">

                            <button
                                type="submit"
                                class="w-full sm:w-auto bg-gray-800 text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-gray-700 active:bg-gray-900 transition shadow-sm whitespace-nowrap">
                                        ...
                                        <div class="flex items-center gap-3 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-[9px] sm:text-[10px] text-gray-500">
                                                    Diterima:
                                                    {{ \Carbon\Carbon::parse($item->received_at)->format('H:i:s') }}
                                                </p>
                                    ...
                                    <div class="bg-white p-3 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between gap-3">
                                        ...
                                        <div class="flex items-center gap-3 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-[9px] sm:text-[10px] text-amber-600 font-semibold">
                                                    Menunggu Scan...
                                                </p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function processScan(scannedCode) {
        ...
        if (isScanning) {
            ...
            logDebug(
                'Skipped, still processing previous scan.',
                'warn'
            );
        ...
        .then(result => {
            ...
            if (
                ...
                logDebug(
                    `Server Response: Success - ${result.body.message}`
                );
                ...
                result.status === 400 ||
                result.body.status === 'warning'
            ) {

                logDebug(
                    `Server Response: Warning - ${result.body.message}`,
                    'warn'
                );
                ...
                logDebug(
                    `Server Response: Error - ${result.body.message}`,
                    'error'
                );
    ...
    function onScanSuccess(decodedText, decodedResult) {
        ...
        logDebug(
            `Scan Success! Captured: ${decodedText}`
        );
    ...
    const manualForm =
        document.getElementById('manual-scan-form');

    const manualInput =
        document.getElementById('manual-resi');
