...
<!-- Daftar Petugas Lapang -->
                    <a href="{{ url($prefix . '/daftar-petugas-lapang') }}" onclick="closeSidebarOnMobile()"
                        class="flex items-center gap-3 px-3 sm:px-4 py-3 rounded-xl text-sm font-semibold text-gray-600 hover:bg-green-50 hover:text-green-700 transition-all group">
                    ...
                    @if ($toko && !empty($toko->latitude))
                        <div class="px-3 sm:px-4 mt-4">
                            ...
                            @if ($toko->request_reset_lokasi)
                                ...
                                <div
                                    class="flex items-start gap-2.5 p-3 bg-yellow-50 rounded-xl border border-yellow-100">
                                    <svg class="w-4 h-4 text-yellow-500 animate-spin flex-shrink-0 mt-0.5"
                            ...
                            @else
                                ...
                                @php
                                    ...
                                    if ($toko->last_location_set_at) {
                                        $tanggalBisaReset = \Carbon\Carbon::parse($toko->last_location_set_at)->addDays(
                                            30,
                                        );
                                ...
                                @if ($bisaReset)
                                    ...
                                    <button onclick="ajukanResetLokasi()"
                                        ...
                                        <span
                                            class="text-xs font-bold text-gray-600 group-hover:text-blue-600 transition-colors">
                                            Ajukan Reset Lokasi
                                        </span>
                                    </button>
                                @else
                                    ...
                                    <button disabled
                                        class="w-full flex flex-col items-center justify-center p-2.5 bg-gray-50 border border-gray-200 rounded-xl cursor-not-allowed">
    ...
    <script>
        ...
        async function ajukanResetLokasi() {
            if (!confirm(
                    'Apakah Anda yakin ingin mengajukan perubahan titik lokasi toko? Proses ini butuh persetujuan Admin.'
                )) return;
