...
<body class="bg-gray-100 font-sans min-h-screen flex items-center justify-center p-4 md:p-8">
    ...
    <div class="bg-white w-full max-w-5xl rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[95vh]">
        ...
        <div
            class="hidden md:flex w-1/2 bg-green-600 p-12 flex-col justify-center relative overflow-hidden text-white min-h-[600px]">
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-10">
                    <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
                        <path
                            d="M11 3a1 1 0 100 2h2.586l-6.293 6.293a1 1 0 10-1.414 1.414L12 6.414V9a1 1 0 102 0V4a1 1 0 00-1-1h-5z">
                        </path>
                    </svg>
                    <span class="text-2xl font-bold tracking-tight">PT.Sage Mashlahat Indonesia </span>
                </div>
                <h1 class="text-4xl lg:text-5xl font-bold leading-tight mb-4">Efisiensi Distribusi<br>di Ujung Jari.
                </h1>
                <p class="text-green-100 text-lg">Platform manajemen alokasi dan retur benih terpadu untuk efisiensi
                    operasional.</p>
            </div>
        ...
        <div
            class="w-full md:w-1/2 p-6 md:p-10 flex flex-col items-center relative bg-white overflow-y-auto no-scrollbar">
            ...
            <div class="w-full max-w-sm my-auto flex flex-col items-center pb-8">
                ...
                <div class="flex bg-gray-100 p-1 rounded-xl mb-6 w-full">
                    <button onclick="switchTab('toko')" id="btnToko"
                        class="flex-1 py-2 text-sm font-bold bg-white text-gray-800 shadow-sm rounded-lg transition">Toko
                        (QR)</button>
                    <button onclick="switchTab('manajemen')" id="btnManajemen"
                        class="flex-1 py-2 text-sm font-bold text-gray-500 hover:text-gray-700 rounded-lg transition">Manajemen</button>
                </div>
                ...
                <div id="tabToko" class="w-full flex flex-col items-center animate-fade-in">
                    ...
                    <p class="text-sm text-gray-500 text-center mb-6">Arahkan kamera ke QR Code yang tersedia di dinding
                        toko untuk mulai mengelola retur.</p>

                    <div
                        class="relative w-56 h-56 bg-gray-900 rounded-2xl shadow-inner mb-6 flex-shrink-0 flex items-center justify-center">
                        <div id="reader" class="absolute inset-0 rounded-2xl overflow-hidden"></div>

                        <div class="scanner-corner border-t-0 border-r-0 top-4 left-4 rounded-tl-lg hidden"
                            id="scanAnim1"></div>
                        <div class="scanner-corner border-t-0 border-l-0 top-4 right-4 rounded-tr-lg hidden"
                            id="scanAnim2"></div>
                        <div class="scanner-corner border-b-0 border-r-0 bottom-4 left-4 rounded-bl-lg hidden"
                            id="scanAnim3"></div>
                        <div class="scanner-corner border-b-0 border-l-0 bottom-4 right-4 rounded-br-lg hidden"
                            id="scanAnim4"></div>
                        <div class="scanner-line hidden" id="scanAnim5"></div>

                        <span id="cameraPlaceholder" class="text-xs text-gray-400 z-10 relative text-center px-4">Kamera
                            belum aktif.<br>Tekan tombol di bawah.</span>
                    </div>

                    <div class="w-full space-y-4">
                        <button type="button" id="btnStartScan"
                            class="w-full bg-green-500 text-white font-bold py-3 rounded-xl hover:bg-green-600 transition shadow-md flex justify-center items-center gap-2">
                        ...
                        <div class="relative flex py-1 items-center">
                            ...
                            <span
                                class="flex-shrink-0 mx-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Atau</span>
                            <div class="flex-grow border-t border-gray-200"></div>
                        </div>

                        <button
                            class="w-full bg-white border border-gray-200 text-gray-700 font-bold py-3 rounded-xl hover:bg-gray-50 transition flex justify-center items-center gap-2">
                    ...
                    <form action="{{ url('/login') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required
                                class="w-full border border-gray-300 px-4 py-3 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition"
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1.5">
                                ...
                                <a href="javascript:void(0)"
                                    onclick="alert('Silakan hubungi administrator IT PT Sage Mashlahat untuk mereset kata sandi Anda.')"
                                    class="text-xs font-semibold text-green-600 hover:underline">Lupa sandi?</a>
                            </div>
                        ...
                        <button type="submit"
                            class="w-full bg-green-600 text-white font-bold py-3.5 rounded-xl hover:bg-green-700 transition shadow-md mt-6">
                            Masuk ke Dashboard
                        </button>
                    </form>
                </div>
    ...
    <script>
        ...
        function startScanner() {
            ...
            const onError = (errorMessage) => {
                // Abaikan error saat frame berjalan mencari QR
            };
            ...
            const onCameraFailed = (err) => {
                // Menampilkan error asli agar kita tahu detail masalahnya
                alert("Gagal membuka kamera. Detail: " + err);
                stopScannerUI();
            };

            // --- LOGIKA PEMANGGILAN KAMERA ---
            // 1. Coba cara standar (kamera belakang)
            html5QrCode.start({
                    facingMode: "environment"
                }, config, onSuccess, onError)
                .then(onCameraStarted)
                .catch((err) => {
                    console.warn("Mode 'environment' gagal, mencoba mencari daftar kamera ID...", err);

                    // 2. Jika gagal, cari daftar kamera fisik secara manual
                    Html5Qrcode.getCameras().then(devices => {
                        if (devices && devices.length > 0) {
                            // Ambil kamera terakhir di daftar (biasanya ini lensa utama belakang di HP)
                            let cameraId = devices[devices.length - 1].id;

                            html5QrCode.start(cameraId, config, onSuccess, onError)
                                .then(onCameraStarted)
                                .catch((fallbackErr) => onCameraFailed(fallbackErr));
                        } else {
                            onCameraFailed("Tidak ada hardware kamera yang terdeteksi.");
                        }
                    }).catch((deviceErr) => {
                        onCameraFailed(deviceErr);
                    });
                });
        }
